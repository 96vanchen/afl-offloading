    ...
    def combine_agents_reward_based(self, main_agent, agents):
        for i in range(self.args.n_agents):
            for main_param, agent_param in zip(main_agent.policy.critic_network.parameters(),
                                               agents[i].policy.critic_network.parameters()):
                if i == 0:
                    main_param.data.copy_(agent_param * (1 / self.args.n_agents))
                else:
                    main_param.data.copy_(main_param + agent_param * (1 / self.args.n_agents))  
    ...
    def run(self):
        ...
        while i < self.episode_limit:
            ...
            while j < self.map_mec.T:
                ...
                reward = 0.7 * reward_coo + 0.3 * reward_single
        ...
        self.ep_reward_list = np.mean(self.ep_reward_list, axis=1)
