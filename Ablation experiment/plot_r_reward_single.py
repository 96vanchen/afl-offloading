...
font = {
        ...
        'size': 16}
plt.rc('font', **font)
plt.plot(x2, reward_mean_0, label='$agent 1$', color='b', linestyle=':', linewidth=1.5)
plt.plot(x2, reward_mean_1, label='$agent 2$', color='b', linestyle='--', linewidth=1.5)
plt.plot(x2, reward_mean_2, label='$agent 3$', color='b', marker='o', linewidth=1.5)
plt.xlabel('Episodes')
plt.ylabel('Reward')
plt.legend()
plt.tight_layout()
plt.show()
# plt.savefig('ablation-reward-single.pdf', dpi=300)